// ===== STATE =====
const DB = {
  get classes() { return JSON.parse(localStorage.getItem('ct_classes') || '["Grade 10-Rizal","Grade 10-Bonifacio","Grade 11-STEM"]'); },
...
function seedData() {
  ...
  const names = [
    'Maria Santos','Juan dela Cruz','Ana Reyes','Carlo Mendoza','Bianca Torres',
    'Miguel Villanueva','Sophia Garcia','Kevin Lim','Isabella Cruz','Rafael Aquino',
    'Camille Ramos','Daniel Tan','Alyssa Navarro','Patrick Chan','Nicole Bautista',
    'Joshua Ocampo','Jasmine Perez','Adrian Flores','Mika Lopez','Christian Sy'
  ];
  const classes = DB.classes;
  const students = names.map((n, i) => ({
    id: `2024-${String(i+1).padStart(4,'0')}`,
    name: n,
    class: classes[i % classes.length],
    email: n.toLowerCase().replace(' ','.')+`@school.edu`,
    phone: ''
  }));
  DB.students = students;

  // Seed 14 days of attendance records
  ...
  for (let d = 13; d >= 0; d--) {
    ...
    for (const cl of classes) {
      const classStudents = students.filter(s => s.class === cl);
      classStudents.forEach(s => {
        const r = Math.random();
        const status = r < 0.80 ? 'present' : r < 0.93 ? 'absent' : 'late';
        records.push({ studentId: s.id, date: dateStr, class: cl, status, subject: '', remarks: '' });
      });
    }
  }
  DB.records = records;
...
function populateClassSelects() {
  ...
  ['attendanceClassSelect','filterClass','reportClass','qrClass','studentClass'].forEach(id => {
